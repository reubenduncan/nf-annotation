#!/usr/bin/env python3
"""
generate_report.py  –  Read combined_results.parquet and produce a
self-contained HTML report in the nf-annotation dark-theme style.
No LLM interpretation — data, charts, and tables only.

Usage:
    generate_report.py <combined_results.parquet>
"""
...
# ── Args ──────────────────────────────────────────────────────────────────────
PARQUET = sys.argv[1] if len(sys.argv) > 1 else 'combined_results.parquet'
OUTFILE = 'annotation_report.html'

PALETTE = [
    '#5b8dee', '#e05c8a', '#4ecdc4', '#f7b731', '#af52de',
    '#ff6b6b', '#a8e063', '#fd9644', '#5b8dee', '#e05c8a',
    '#4ecdc4', '#f7b731', '#af52de', '#ff6b6b', '#a8e063',
]
...
def make_table(d, cols=None, max_rows=500):
    ...
    if len(d) > max_rows:
        rows.append(
            f'<p class="muted" style="font-size:11px;padding:4px 0 16px">'
...
# ── Overview ──────────────────────────────────────────────────────────────────
OV_LABELS = ['AMRFinder', 'CARD RGI', 'KofamScan', 'PlasmidFinder',
             'IntegronFinder', 'mob_suite', 'ISEScan']
OV_KEYS   = ['amrfinder', 'card_rgi', 'kofam', 'plasmidfinder',
             'integronfinder', 'mob_suite', 'isescan']
...
# ── IntegronFinder ────────────────────────────────────────────────────────────
intf_type_l, intf_type_v = top_counts(
    gcol(intf, 'annotation_type', 'type', 'Type', 'element_type'), 10
)
...
def build_overview():
    cards = (
        ...
        + stat_card(OV_COUNTS[3], 'Plasmid Replicons', 'PlasmidFinder',
                    'ok' if OV_COUNTS[3] == 0 else 'warn')
        + stat_card(OV_COUNTS[4], 'Integrons', 'IntegronFinder',
                    'ok' if OV_COUNTS[4] == 0 else 'warn')
        + stat_card(OV_COUNTS[5], 'Plasmid Contigs', 'mob_suite')
        + stat_card(OV_COUNTS[6], 'IS Elements', 'ISEScan',
                    'warn' if OV_COUNTS[6] > 0 else 'ok')
    )
    rows = ''.join(
        f'<tr><td>{lbl}</td><td>{cnt:,}</td></tr>'
        for lbl, cnt in zip(OV_LABELS, OV_COUNTS)
    )
    summary_table = (
        '<div class="table-wrap"><table>'
        '<thead><tr><th>Tool</th><th>Annotation Rows</th></tr></thead>'
...
def build_amr():
    ...
    amr_cols = [
        'Gene symbol', 'Sequence name', 'Class', 'Subclass', 'Method',
    ...
    return (
        ...
        + '<div class="card-grid">' + cards + '</div>'
        + two_col(
            chart_box('amrClassDonut', 'Drug class distribution'),
...
def build_card():
    ...
    rgi_cols = [
        ...
        'AMR Gene Family', 'Best_Identities', 'Cut_Off', 'Contig',
    ]
    return (
        section_open('card')
        + '<h2>CARD RGI</h2>'
        ...
        + '<div class="card-grid">' + cards + '</div>'
        + two_col(
            chart_box('rgiMechDonut', 'Resistance mechanism'),
...
def build_plasmids():
    cards_pf = (
        ...
        + stat_card(
            len(set(str(v) for v in gcol(pf, 'match_name') if str(v).strip())),
    ...
    cards_mob = (
        ...
        + stat_card(
            len(set(str(v) for v in gcol(mob, 'mob_type', 'MOB_type') if str(v).strip())),
            'MOB Types', '',
        )
    )
    ...
    return (
        ...
        + two_col(
            chart_box('pfDonut',   'Replicon type distribution'),
...
def build_is():
    ...
    return (
        ...
        + '<div class="card-grid">' + cards + '</div>'
        + two_col(
            chart_box('iseFamDonut',  'IS family distribution'),
...
# NOTE: f-string backslashes in expressions are forbidden pre-Python 3.12;
# use a helper variable instead.
tags_html = '<div class="tag-row">' + ''.join(
    '<span class="tag' + (' hi' if hi else '') + '">' + t + '</span>'
